...
const Packaging = () => {
  ...
  const deleteAction = (data) => {
    axios
      .delete(process.env.REACT_APP_API_URL + "/packages", {
        headers: { Authorization: localStorage.getItem("token") ?? null },
        data: { _id: [data._id] },
      })
      .then((response) => {
                  ...
                  <CTableBody>
                    {data.docs?.map((product, index) => (
                      <CTableRow key={product.id}>
                        ...
                        <CTableDataCell>
                          {DateTime.fromISO(product.date).toFormat(
                            "yyyy LLL dd"
                          )}
                        </CTableDataCell>
                        <CTableDataCell>
                          {product?.sales_order?.sale_details.total}
                        </CTableDataCell>
                  ...
                  <CCol xs={12}>
                    ...
                    <CRow className="row g-3 px-3 mt-1 mb-5">
                      ...
                      <CCol md={6}>
                        <CFormSelect
                          id="inputState"
                          floatingLabel="Packaging Status"
                          {...register("status")}
                        >
                          <option value="">...</option>
