...
const Packaging = () => {
  ...
  const deleteAction = (data) => {
    axios.delete(process.env.REACT_APP_API_URL + "/packages",
  ...
  const reload = async () => {
    ...
    return await axios
      .get(process.env.REACT_APP_API_URL + "/packages", { params: { page }, headers: { Authorization: localStorage.getItem('token') ?? null } })
      .then((res) => {
  ...
  return (
    <CRow>
      ...
      <CCol xs={12}>
        <CButton color="info" onClick={() => { addForm() }} className="mb-4 text-white">Add Package</CButton>
