...
const Receviables = () => {
  ...
  const [products, setReceviables] = useState([]);
  const [categories, setCategory] = useState({});
  const [salesExecutives, setSalesExe] = useState({});
  const [vendors, setVendors] = useState({});
  const [packageData, setReceviable] = useState({});
  ...
  const create = (data) => {
    axios.post(process.env.REACT_APP_API_URL + "/receivables",
      { payload: data },
  ...
  const updateData = (data) => {
    ...
    data.receivable = packageData.receivable;

    axios.patch(process.env.REACT_APP_API_URL + "/receivables",
      { payload: data },
      { headers: { Authorization: localStorage.getItem('token') ?? null } })
      .then((response) => {
        reload();
  ...
  const deleteAction = (data) => {
    axios.delete(process.env.REACT_APP_API_URL + "/receivables",
  ...
  const reload = async () => {
    ...
    return await axios
      .get(process.env.REACT_APP_API_URL + "/receivables", { params: { page }, headers: { Authorization: localStorage.getItem('token') ?? null } })
      .then((res) => {
  ...
  const onEdit = (data) => {
    ...
    setValue('receivable_no', data.receivable_no)
  ...
  return (
    <CRow>
      ...
      <CCol xs={12}>
        {/* <CButton color="info" onClick={() => { addForm() }} className="mb-4 text-white">Add Receviable</CButton> */}
          ...
          <CCardBody>
            ...
            <CTable align="middle" className="mb-0 border" hover responsive>
              <CTableHead color="dark">
                <CTableRow>
                  ...
                  <CTableHeaderCell scope="col"># Receivables No</CTableHeaderCell>
                  <CTableHeaderCell scope="col">Associated Purchase </CTableHeaderCell>
              ...
              <CTableBody>
                {data.docs?.map((product, index) =>
                  <CTableRow key={product.id}>
                    ...
                    <CTableDataCell>#{product.receivable_no}</CTableDataCell>
                    <CTableDataCell>
                      <Link to={`/purchase-pipeline/${product?.purchase_order?._id}`} >#{product?.purchase_order?.order_no}</Link>
                    </CTableDataCell>
                    <CTableDataCell>{DateTime.fromISO(product.date).toFormat('yyyy, LLL dd')}</CTableDataCell>
                    <CTableDataCell>{product?.purchase_order?.sale_details.total}</CTableDataCell>
            ...
            <div className='mt-2 px-2 float-end'>
              <Pagination
                ...
                href="http://localhost:3000/receivables?page=*"
                pageOneHref="http://localhost:3000/receivables"
              />
            </div>
              ...
              <CForm onSubmit={handleSubmit(onFormSubmit, onErrors)}>
                ...
                <CModalBody>
                  <CCol xs={12}>
                    ...
                    {/* Show Receviable Form  */}
                    <CRow className="row g-3 px-3 mt-1 mb-5">
                      <ValidationAlert validate={{ visible: validationAlert, errorObjData }} />
                      <CCol md={6}>
                        <CFormInput type="text" id="inputEmail4" disabled floatingLabel="Receivable No#" {...register("receivable_no")} />
                      ...
                      <CCol md={6}>
                        <CFormSelect id="inputState" floatingLabel="Receviables Status"{...register("status")}>
                          <option value="">...</option>
                                  ...
                                  return (
                                    <tr id='addr0'>
                                      ...
                                      <td>
                                        ...
                                        <div><span className='text-secondary'><b>sku: </b>{element.product_id?.sku}</span></div>
                      ...
                      <CCol md={12}>
                        <CFormTextarea id="cost_data" floatingLabel="Additional Notes" style={{ height: '100px' }} {...register("additional_notes")} rows="6">
                        </CFormTextarea>
                      </CCol>

                      <CCol md={12} className="mt-4">
                        <div className='float-end'>
                          <input type="hidden"  {...register("purchase_order")} value={packageData.purchase_order?._id}></input>
