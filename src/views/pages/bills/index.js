...
const Bill = () => {
  ...
  const updateData = (data) => {
    ...
    axios.patch(process.env.REACT_APP_API_URL + "/bills",
      { payload: data },
      { headers: { Authorization: localStorage.getItem('token') ?? null } })
      .then((response) => {
        reload();
  ...
  const deleteAction = (data) => {
    axios.delete(process.env.REACT_APP_API_URL + "/bills",
  ...
  const reload = async () => {
    ...
    return await axios
      .get(process.env.REACT_APP_API_URL + "/bills", { params: { page }, headers: { Authorization: localStorage.getItem('token') ?? null } })
      .then((res) => {
        ...
        <CCard className="mb-4">
          ...
          <CCardBody>
            ...
            <CTable align="middle" className="mb-0 border" hover responsive>
              ...
              <CTableBody>
                {data.docs?.map((bill, index) =>
                  <CTableRow key={bill.id}>
                    <CTableHeaderCell scope="row">{index + 1}</CTableHeaderCell>
                    <CTableDataCell>#{bill.bill_no}</CTableDataCell>
                    <CTableDataCell>{DateTime.fromISO(bill.bill_date).toFormat('yyyy/MM/dd')}</CTableDataCell>
                    <CTableDataCell>
                      <Link to={`/sales-pipeline/${bill?.sales_order?._id}`} >#{bill?.purchase_order?.order_no}</Link>
                    </CTableDataCell>
                    <CTableDataCell>{bill?.sale_details.total}</CTableDataCell>
                    <CTableDataCell>{bill.status}</CTableDataCell>
                    <CTableDataCell>{bill.payment}</CTableDataCell>
                    <CTableDataCell>
                      <CTooltip
                        content="Edit"
                        placement="top"
                      >
                        <CButton color="info" onClick={() => onEdit(bill)} className="me-md-2"><CIcon className="text-white" size={'lg'} icon={cilPencil} /></CButton>
                      </CTooltip>
                      <CTooltip
                        content="Delete"
                        placement="top"
                      >
                        <CButton color="danger" onClick={() => onDelete(bill)} className="me-md-2"><CIcon className="text-white" size={'lg'} icon={cilTrash} /></CButton>
                      </CTooltip>
                    </CTableDataCell>
                  </CTableRow>
                )}
