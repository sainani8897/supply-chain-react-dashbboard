...
import { cilPencil, cilSitemap, cilTrash, cilWarning } from "@coreui/icons";
...
const PurchaseOrder = () => {
  ...
  const deleteAction = (data) => {
    axios
      .delete(process.env.REACT_APP_API_URL + "/purchase-order", {
        headers: { Authorization: localStorage.getItem("token") ?? null },
        data: { _id: [data._id] },
      })
      .then((response) => {
  ...
  const reload = async () => {
    ...
    return await axios
      .get(process.env.REACT_APP_API_URL + "/purchase-order", {
        params: { page },
        headers: { Authorization: localStorage.getItem("token") ?? null },
      })
      .then((res) => {
  ...
  const onEdit = (data) => {
    ...
    setValue(
      "delivery_date",
      DateTime.fromISO(data.delivery_date).toFormat("yyyy-MM-dd")
    );
                  ...
                  <CTableBody>
                    {data.docs?.map((product, index) => (
                      <CTableRow key={product.id}>
                        ...
                        <CTableDataCell>
                          {product.vendor_id?.display_name}
                        </CTableDataCell>
                    ...
                    <CRow className="row g-3 px-3 mt-1 mb-5">
                      ...
                      <CCol md={6}>
                        <CFormInput
                          type="date"
                          id="inputPassword4"
                          floatingLabel="Purchase Order Date"
                      ...
                      <CCol md={6}>
                        <CFormInput
                          ...
                          {...register("delivery_date", options.delivery_date)}
                        />
                      </CCol>
